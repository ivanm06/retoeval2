                ...
                <main class="flex flex-row">
                    <section id="articulos" class="grow flex flex-row flex-wrap gap-10 justify-center items-center p-10 pt-20 pb-40 items-start">
                        <xsl:for-each select="//articulo">
                            <div class="articulo w-[40vh] h-fit bg-[#eee] min-h-100 rounded-md shadow-md hover:scale-105 has-[.fs]:hover:scale-100 cursor-pointer flex flex-col relative" data-tipo="{tipo}" data-e="{establecimiento}">
                                <!-- Mostrar fuera de stock. -->
                                <xsl:if test="stock &lt;= 0">
                                    <div class="fs w-full h-full absolute z-[4] bg-red-800/30 hover:bg-red-800/35 cursor-auto rounded-lg group-has-[fs]:pointer-events-none"></div>
                                </xsl:if>
                                <!-- -->
                                <div class="relative bg-[#fdfdfd] min-h-[50%] flex items-center justify-center">
                                    <img class="rounded-md bg-[#fdfdfd] w-auto max-h-[40vh] place-self-center" src="./images/articulos/articulo{id}.png" alt="articulo" onerror="this.onerror=null; this.src='./images/placeholder.png';" />
                                    <div class="absolute top-1 right-1 px-2 flex justify-between items-center w-full">
                                        <div class="justify-end flex">
                                            <xsl:choose>
                                                <xsl:when test="tipo = 'snowboard'">
                                                    <div class="flex gap-2 w-fit">
                                                        <span class="p-[2px] px-2 rounded-xl bg-black/20"><xsl:value-of select="nivel"/></span>
                                                        <span class="p-[2px] px-2 rounded-xl bg-black/20"><xsl:value-of select="modalidad"/></span>
                                                    </div>
                                                </xsl:when>
                                                <xsl:when test="tipo = 'ski'">
                                                    <span class="p-[2px] px-2 rounded-xl bg-black/20"><xsl:value-of select="modalidad"/></span>
                                                </xsl:when>
                                                <xsl:when test="tipo = 'accesorio'">
                                                    <span class="p-[2px] px-2 rounded-xl bg-black/20"><xsl:value-of select="tipoAccesorio"/></span>
                                                </xsl:when>
                                                <xsl:otherwise>Fuera de stock</xsl:otherwise>
                                            </xsl:choose>
                                        </div>
                                        <span class="bg-black/20 rounded-md px-2">
                                            <xsl:choose>
                    ...
                    <!-- Filtros -->
                    <div class="filtros select-none fixed bottom-10 flex justify-between bg-[#222] shadow-xl shadow-white/5 gap-4 text-white p-4 px-10 rounded-full left-1/2 transform -translate-x-1/2 -translate-y-1/2">
